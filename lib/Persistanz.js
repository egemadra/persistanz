...
class Persistanz {
  ...
  constructor (dbConfig, options) {
    ...
    */
    ...
    this.ormErrors = [];
    this.ignoreTables = null; //[] of table names.
    this.separator = '.';
    this.separatorRegex = null; //will be fixed in create.
  }
  ...
  */
  create (cb) {
    var fn = co.wrap(function * (pers) {
      ...
      if (["mysql", "mysql2", "postgres", "sqlite3"].indexOf(dbConfig.adapter) < 0)
        throw new Error("Database adapter is not specified or not supported.");

      //extract schema information:
      var rawSchema = yield schemax.extract(dbConfig);
      pers.ignoreTables = helper.identifyIgnoreTables(rawSchema, pers.options);
      //make adjustments to the raw schema for easier use:
      pers.schema = helper.persifyRawSchema(rawSchema, pers.ignoreTables, pers.ormErrors);
      ...
      //separator:
      if (pers.options.separator != null)
        pers.separator = helper.checkSeparator(pers.options.separator);
      pers.separatorRegex = new RegExp("([^\\\\])\\" + pers.separator, "g");
  ...
  * @param [fieldList='*'] {string|Array} fields A comma separated field list or an array of fields to be added in the select clause.
  * @param {function=} cb An optional callback in the (err, result) signature.
  ...
  * @param [fieldList='*'] {string|Array} A comma separated field list or an array of fields to be added in the select clause.
  * @param {function=} cb An optional callback in the (err, result) signature.
  ...
  */
  ...
  _hydrate (tx, object, fieldList, cb) {
    var fn = co.wrap(function *(pers){
      ...
      var fresh = yield pers._loadById(tx, modelMeta.name, pkValue, fieldList);
      if (!fresh)
        throw new Error("Object cannot be hydrated because it is not found in the database.");

      //copy all object properties:
      return Object.assign(object, fresh);
    });

    return helper.polycallTx(tx, fn, cb, null, this);
  }
  ...
  */
  ...
  _deleteById(tx, modelName, id, cb) {
    var object = this._castOrError(tx, {}, modelName, cb);
  ...
  _castOrError (tx, fromObject, modelName, cb) {
    var o;
    try { o = this.cast(fromObject, modelName); }
    catch(err) {
      tx && tx.isActive() && tx.rollback();
      if (cb) { cb(err); return; }
  ...
  _loadById (tx, modelName, id, fields, cb) {
    var fn = co.wrap(function *(pers){
      ...
      var isEmpty = fields == null || fields === '' || (Array.isArray(fields) && ! fields.length);
      var select = isEmpty ? '*' : fields;
  ...
  _saveAs (tx, object, modelName, forceInsert, cb) {
    var o = this._castOrError(tx, object, modelName, cb);
  ...
  _save (tx, object, forceInsert, cb) {
    var fn = co.wrap(function *(pers) {
      ...
      var isInsert = pkValue === undefined || forceInsert;
      var isAutoIncrement = modelMeta.table.columns[pkName].isAI;

      //reject inserts without a set pk and non-autoincrement:
      if (isInsert && pkValue === undefined && ! isAutoIncrement)
        throw new Error("Can't insert without the primary key is set when the table has not an auto-increment pk.");
      ...
      if (typeof object.beforeSave === 'function')
        continueSave = yield helper.polycallBasedOnSignatureLength(object.beforeSave, object, [tx, ret.command], 3);
      ...
      for (var columnName in table.columns) {
        if (object[columnName] !== undefined) {
          ...
          if (isInsert && modelMeta.serialization[columnName]
              && modelMeta.serialization[columnName].default != null) {
                saveProps.push(pers.escapeId(columnName));
      ...
      if (modelMeta.discriminator != null && object[modelMeta.discriminator] == undefined) {
        ...
        values.push(modelMeta.serialize(modelMeta.discriminator, modelMeta.name));
      }
      ...
      if (isInsert) {
        ...
        values.push(modelMeta.serialize(pkName, pkValue));
      ...
      if (!queryResult.rowCount) ret.status="not-saved";
        ...
        ret.status="saved";
        if (isInsert) {
          //pg always returns the lastInsertId as we used "RETURNING" for inserts.
          if (pers.adapter.name === 'postgres')
            lastInsertId = modelMeta.deserialize(pkName, queryResult.lastInsertId);
          else
            lastInsertId = isAutoIncrement ? modelMeta.deserialize(pkName, queryResult.lastInsertId) : object[pkName];
      ...
      if (typeof object.afterSave === 'function' && ret.status==="saved")
        yield helper.polycallBasedOnSignatureLength(object.afterSave, object, [tx, ret.command], 3);
