...
function Persistanz(dbConfig, options)
{
  ...
  */
  ...
  /**
  * Alias for [Persistanz.query()](#Persistanz#query)
  * @name Persistanz.q
  */
  this.q=this.query;
  ...
  */
	...
	{
    if (!tx) throw new Error("No transaction object supplied for insertX.");
  ...
  */
	...
	{
    if (!tx) throw new Error("No transaction object supplied for saveX.");
  ...
  */
	...
	{
    if (!tx) throw new Error("No transaction object supplied for saveAsX.");
  ...
  */
	...
	{
    if (!tx) throw new Error("No transaction object supplied for insertAsX.");
  ...
  */
	...
	{
    if (!tx) throw new Error("No transaction object supplied for deleteByIdX.");
  ...
  */
    ...
    if (!tx) throw new Error("No transaction object supplied for deleteObjectX.");
  ...
  */
  ...
  //TODO: WE assumed there is always a primary key, this is not ok, fix it.
	function save(tx, object, forceInsert, cb)
	{
		var fn=co.wrap(function *() {
			...
			for (var propName in cls.props)
			{
				...
				if (!prop.boundFk && object[propName]!==undefined)
				{
	...
	function loadById(tx, modelName, id, fields, cb)
	{
		...
		return pers.q(tx).f(modelName).w("{"+pkName+"}=?", id).l(1).s(select).exec()
  ...
  function handleToManyConfiguration()
  {
	...
	function generateModel(name, baseModel)
	{
		...
		f.deleteById=function(id, cb){return pers.deleteById(name, cb);}
		...
		f.deleteByIdX=function(tx, id, cb){return pers.deleteByIdX(tx, name, cb);}
