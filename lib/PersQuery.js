...
{
  if (!aQuery.parentQuery) return aQuery;
  return getRootQuery(aQuery.parentQuery);
}
...
function groupRows(q, mappedRows)
{
  ...
  for (var i in mappedRows)
  {
    ...
    if (!groupedRows.has(row[foreignKey]))
      groupedRows.set(row[foreignKey],[]);
    groupedRows.get(row[foreignKey]).push(row);
  }
  return groupedRows;
}

/**
* Makes the query so that results are indexed by one the fields/columns of
* the model/table.
*
* Sometimes you need to have the result set of a query indexed by the values of a certain field.
* This happens when you have to compare or filter the result set against another array of values,
* so instead of looping both sets, you only loop one set (indexless set) and get
* the matching items from the indexed set. indexField is one field from the select
* part of query in the format used by the .select(), i.e, fieldList. You want this field to be
* usually the primary key but any field with a unique value (e.g email from Users table)
* is fine. Note that, if the value is not unique, you cannot receive the
* entire result set because the rows with the same index value will overwrite
* the previous one.
*
...
* are ids.
*
* The ORDER BY clause is respected, so the order of items
* in the Map are as you would expect. You must include the the field in the
* fieldExpression into the select clause of the query, or it won't work.
...
* For short **.sa**
*
* Adds to the SELECT clause of the query, but uses [curlyExpression](#quot-curly-expression-quot) syntax, as described in the .where.
...
PersQuery.prototype.toMany=function(toManyProp, mountPoint)
{
...
/**
* Registers an event callback to the query and renders it an "evented query".
*
* This is intended for queries returning large result sets.
* Instead of one huge array in memory, it allows you to get rows as they come
* from the database. Following event types are defined and can be registered
* by calling this method multiple times, as many times for each:
*
*   **"object"**: Callback is called with an object (a row). Callback has (object, index) signature. Index is index value of the row if you called .index() method before exec.
*
...
*
* Besides these, each underlying db engine may have their own events. Mysql for
* example has "fields" event. You can listen to those events too. Persistanz doesn't know
* about them, and simply passes them to the database engine. If you register them and the
* database fires those events, the associated callback will be called.
*
* Important thing to note is that once you called this method, even for once, .exec() return is modified.
* It still returns .count member if you added a "count" event, but objects field is _true_ if no error occured.
* If no .count() called, the whole return value is _true_. If an error occured, however, your .exec() callbak
* (or your rejected promise) contains the error, even if you defined an "error" event.
* In short, error and calc results are returned from the .exec() regardless of whether there are events registered to catch them.
...
PersQuery.prototype.exec=function(cb)
{
  ...
  var fn=co.wrap(function*()
  {
    ...
    {
      if (hasChildQueries) throw new Error("Evented queries may not have toMany fields.");
...
PersQuery.prototype._processWhere=function()
{
  ...
  for (var i in p.get(this).dsl.whereValues)
  {
    ...
    //we build the subquery and replace the question mark with it ourselves, so that
    //any-db engine doesn't escape it. We have to be careful with the ? order and
    //ony replace the right ?. This changes "where" params array, so we create a new
    //one by pushing normal params that are to be escaped.
