...
class PersQuery {
  ...
  constructor(opts) {
    ...
    this.abstractAffixes = Object.assign({}, this.pers.abstractAffixes); //create a copy.
  ...
  *
  * @param {string|function} indexField A field conformant to the [fieldList](#PersQuery#select) expression or a callback function that accepts one object from the query result set.
  * @return {persistanz.PersQuery} The query object itself.
  */
  ...
  *
  * fieldList is either an array or a comma separated string of fields. Any whitespace is trimmed. A field can be one of the following:
  *
  * * A real column name in the table,
  * * \*: to select all columns just as in normal SQL.
  * * dot (or a custom separator) separated bridge field names with final part being either a real column name or \*. For example "customer.name", "product.brand.\*".
  * * dot (or a custom separator) separated toMany field names. For example: "customer.orders.\*".
  * * A negation expression starting with the ! character. This is to negate the expression, meaning that the columns
  * generated by the expression won't be present in the select clause of the sql (nor in the returned objects).
  ...
  * laboriously listing all columns except the password. Works on bridge field names too, for example:
  * "customer.\*, !customer.password, !customer.passwordResetToken".
  * * A partial field name understood by the "field abstraction over affix" mechanism.
  *
  ...
  * @chainable
  * @param [fieldList="*"] {string|Array} fieldList A comma separated field list or an array of fields to be added in the select clause.
  * @return {persistanz.PersQuery} The query object itself.
  */
  select (fieldList) {
  ...
  * @param [fieldList="*"] {string|Array} fieldList A comma separated field list or an array of fields to be added in the select clause. See [PersQuery.select](#PersQuery#select).
  * @return {persistanz.PersQuery} The query object itself.
  */
  ...
  * @param {string} toManyExp Only one field from a [fieldList](#field-list) expression where the part after the final separator being a [toMany field](#tomany-field).
  * @return {persistanz.PersQuery} A toMany query.
  */
  toMany (toManyExp) {
    var rootQuery = getRootQuery(this);
    var q = rootQuery.resolver.resolveFieldExpression(toManyExp, "toMany");
  ...
  _createToManyQuery(prefixParts, toManyField, modelMeta) {
    var toManyPrefix = prefixParts.join(this.pers.separator);
    ...
    q.prefixToParent = prefixParts.join(this.pers.separator) || '';
  ...
  */
  exec (cb) {
    var fn = co.wrap(function* (persQuery) {
      persQuery.build();
      var adapter = persQuery.opts.tx || persQuery.pers.adapter
      ...
      var rows, calcCount = null;

      if (evented && hasChildQueries) {
        throw new Error("Streaming queries may not have toMany fields.");
      }

      //we do this first because streaming queries need this value before "end"
      //event.
      if (lq) {
        var calcResult = yield adapter.query(lq.sql, lq.values);
        var calcCount = + calcResult[0].count;
        if (persQuery.events.calc)
          persQuery.events.calc.forEach( cb => cb(calcCount));
      }
        ...
        if (hasChildQueries && rows.length) {
          ...
          for (var childQDef of persQuery.toManyQueries) { //we are looping a Map object
            ...
            */
            ...
            //remoteFkColumnName may have characters that need to be escaped:
            var sep = persQuery.pers.separator;
            var eRFKName = remoteFkColumnName.split(sep).join("\\" + sep);
            childQ.resolver.setDsl("toManyIdsIn", `{${eRFKName}} IN ( ${placeHolders} )`);
    ...
    return helper.polycallTx(this.opts.tx, fn, cb, null, this);
  }
  ...
  *
  * PersQuery instance inherits affixes from the Persistanz object, so it has
  * its own copy of affixes.
  *
  * To clear a particular affix, pass _null_ as type argument.
  * To clear all abstractions pass _null_ as affix argument.
  * @example
  * pers.q().f("Product").affix("special_", "prefix");
  * //if Product doesn't have price column but a special_price
  * //column, the returning object will have a .price column whose
  * //value is filled with the value of special_price.
  ...
  */
  affix (affix, type) {
    ...
    if (affix == null) {
      this.abstractAffixes = {};
      return this;
    }

    if (type === null) {
      delete this.abstractAffixes[affix];
      return this;
    }
