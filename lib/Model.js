...
function Model()
{
  ...
  */
  this.hydrate=function(fieldList, cb){};
  ...
  * @return {Promise} A promise resolving to the same object, but now hydrated.
  */
  this.hydrateX=function(tx, fieldList, cb){};
  ...
  */
  ...
  /**
  * Called by persistanz after a select query (load, loadById, hydrate and X versions)
  *
  * Works like other hooks and is called after the object is created. The return value does
  * not matter and is discarded. You still need to call the callback or resolve
  * a promise so that the program doesn't hang, waiting for your callback to complete.
  *
  * @param {PersTransaction|null} tx A transaction object, if the object is loaded was within a transaction, null if not.
  * @param {function=} cb An optional callback in the (err, result) signature.
  * @return {Promise} A promise resolving to any or no value.
  */
  this.afterLoad=function(tx, cb){}
}
